          ...
          <li key={item.name}>
            <Link
              ...
              className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-300 group ${
                isActive 
                  ? 'bg-gold-text/10 text-gold-text font-bold border-r-2 border-gold-text' 
                  : 'text-neutral-400 hover:text-white hover:bg-white/5'
              }`}
            >
              <item.icon className={`w-5 h-5 transition-transform duration-300 group-hover:scale-110 ${isActive ? 'text-gold-text shadow-[0_0_15px_rgba(230,194,0,0.3)]' : ''}`} />
...
const UserMenu = ({ lang, session }: { lang: string; session: any }) => {
  ...
  return (
    <div className="flex flex-col gap-2">
      <Link 
        href={`/${lang}/profile`}
        className="flex items-center gap-3 w-full p-2 bg-neutral-900/50 hover:bg-neutral-800 rounded-xl transition-all border border-transparent hover:border-gold-text/30 group cursor-pointer"
      >
        {session.user?.image ? (
          <Image src={session.user.image} alt="User" width={36} height={36} className="w-9 h-9 rounded-full border-2 border-gold-text/30 group-hover:border-gold-text/80 object-cover transition-colors" />
        ) : (
          <div className="w-9 h-9 rounded-full border-2 border-gold-text/30 bg-neutral-800 flex items-center justify-center font-bold text-xs">{session.user?.name?.[0] || 'U'}</div>
        )}
        <div className="flex-1 overflow-hidden">
          <p className="text-sm font-bold text-white truncate group-hover:text-gold-text transition-colors uppercase tracking-tight">{session.user?.name}</p>
...
export default function Sidebar({ lang, dict, isMobileDrawer }: SidebarProps) {
  ...
  return (
    <aside className={isMobileDrawer ? "flex flex-col w-full h-full p-2" : "hidden md:flex fixed top-0 w-60 bg-transparent h-screen overflow-y-auto flex-col glass-effect !border-r-0 z-20 pb-32 no-scrollbar"}>
      {!isMobileDrawer && (
        ...
        <nav className="space-y-1">
          ...
          <div className="pt-8 space-y-4">
            {session && (
              <Link
                href={`/${lang}/upload`}
                className="flex items-center gap-3 px-4 py-3 rounded-xl bg-gradient-to-r from-cobalt-deep to-cobalt-light text-white font-semibold hover:brightness-110 transition-all group shadow-lg shadow-cobalt-light/20 active:scale-95"
              >
            ...
            <div className="flex items-center justify-between px-2 pt-4">
              ...
              <div className="flex gap-1.5">
                <Link 
                  ...
                  className={`text-[9px] font-bold px-1.5 py-0.5 rounded transition-all transform hover:scale-110 ${lang === 'es' ? 'bg-gold-text text-black' : 'bg-white/5 text-neutral-500 hover:text-white hover:bg-white/10'}`}
                >
                  ES
                </Link>
                <Link 
                  href={pathname ? pathname.replace(`/${lang}`, '/en') : '/en'}
                  className={`text-[9px] font-bold px-1.5 py-0.5 rounded transition-all transform hover:scale-110 ${lang === 'en' ? 'bg-gold-text text-black' : 'bg-white/5 text-neutral-500 hover:text-white hover:bg-white/10'}`}
                >
                  EN
                </Link>
              </div>
            ...
            <div className="flex gap-4 px-2 mt-6 text-[9px] font-medium text-neutral-600 uppercase tracking-widest border-t border-white/5 pt-4">
              <Link href={`/${lang}/privacy`} className="hover:text-gold-text transition-colors">Privacidad</Link>
              <Link href={`/${lang}/terms`} className="hover:text-gold-text transition-colors">Términos</Link>
            </div>
          </div>
        </nav>
      </div>
